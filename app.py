...
# --- SECTION 2: MARKET RATES & BATTERY SYSTEM ---
st.subheader("2. Market Rates & System Choice | مارکیٹ ریٹ")
...
with col4:
    ...
    # NEW: Battery System Voltage Option
    sys_vol = st.selectbox("Battery System (Volts) / بیٹری سسٹم", ["Auto Select", "12V (1 Battery)", "24V (2 Batteries)", "48V (4 Batteries)"])
...
if st.button("Generate Technical Report | رپورٹ تیار کریں", use_container_width=True):
    ...
    # Voltage and Battery Logic
    if sys_vol == "Auto Select":
        if kw <= 1.2: vol, n_b = 12, 1
        elif kw <= 2.8: vol, n_b = 24, 2
        else: vol, n_b = 48, 4
    else:
    ...
    with res2:
        st.info(f"**Batteries:**\n{n_b} Nos ({vol}V System)")
        st.success(f"**Battery Cost:**\nRs. {n_b * b_r:,.0f}")
        st.warning(f"**Night Backup:**\n{backup_light:.1f} to {backup_heavy:.1f} Hours")
